...
and
nameOfFunction x = match x with Function_Definition(iden, _, _) -> match iden with Identifier_Declaration(_, name) -> name
and
...
(*
and
identifier_eval x currStore = match x with
    | Identifier_Reference(name) -> evalReturn(currStore, (storeLookup currStore name).storedValue)
*)
and
...
if_statement_eval x currStore = match x with
    | If_Statement_If_Bool(exp, statList)                       -> let expEvaluation = expression_bool_eval exp currStore in
                                                                   if match expEvaluation.evaluation with BoolValue(x) -> x = true then
...
io_operation_eval x currStore = match x with
    | IO_Operation_Print_Int(exp)               -> let eval = expression_int_eval exp currStore in print_int (match eval.evaluation with IntValue(x) -> x); eval
    | IO_Operation_Print_Float(exp)             -> let eval = expression_float_eval exp currStore in print_float (match eval.evaluation with FloatValue(x) -> x); eval
    | IO_Operation_Print_Bool(exp)              -> let eval = expression_bool_eval exp currStore in print_bool (match eval.evaluation with BoolValue(x) -> x); eval
    | IO_Operation_Print_String(exp)            -> let eval = expression_string_eval exp currStore in print_string (match eval.evaluation with StringValue(x) -> x); eval
    | IO_Operation_Print_Int_Identifier(exp)    -> let eval = expression_identifier_eval exp currStore in print_int (match eval.evaluation with IntValue(x) -> x); eval
    | IO_Operation_Print_Float_Identifier(exp)  -> let eval = expression_identifier_eval exp currStore in print_float (match eval.evaluation with FloatValue(x) -> x); eval
    | IO_Operation_Print_Bool_Identifier(exp)   -> let eval = expression_identifier_eval exp currStore in print_bool (match eval.evaluation with BoolValue(x) -> x); eval
    | IO_Operation_Print_String_Identifier(exp) -> let eval = expression_identifier_eval exp currStore in print_string (match eval.evaluation with StringValue(x) -> x); eval
and
expression_int_eval x currStore = match x with
    ...
    | Expression_Float_To_Int(exp)      -> let eval = (expression_float_eval exp currStore) in
                                           let FloatValue(fVal) = eval.evaluation in
    ...
    | Expression_Bool_To_Int(exp)       -> let eval = (expression_bool_eval exp currStore) in
                                           let BoolValue(bVal) = eval.evaluation in
    ...
    | Expression_String_To_Int(exp)     -> let eval = (expression_string_eval exp currStore) in
                                           let StringValue(sVal) = eval.evaluation in
    ...
    | Expression_Identifier_To_Int(exp) -> let eval = (expression_identifier_eval exp currStore) in
                                               ...
                                               | FloatValue(fVal)  -> raise (EvaluationError("Expected value of type int but got value of type float"))
                                               | BoolValue(bVal)   -> raise (EvaluationError("Expected value of type int but got value of type bool"))
                                               | StringValue(sVal) -> raise (EvaluationError("Expected value of type int but got value of type string"))
and
expression_float_eval x currStore = match x with
    ...
    | Expression_Int_To_Float(exp)        -> let eval = (expression_int_eval exp currStore) in
                                             let IntValue(iVal) = eval.evaluation in
    ...
    | Expression_Identifier_To_Float(exp) -> let eval = (expression_identifier_eval exp currStore) in
                                                 match eval.evaluation with
                                                 | IntValue(iVal)    -> expression_float_eval (Expression_Int_To_Float(Expression_Int_Literal(match eval.evaluation with IntValue(x) -> x))) eval.newStore
...
expression_bool_eval x currStore = match x with
    ...
    | Expression_Identifier_To_Bool(exp) -> let eval = (expression_identifier_eval exp currStore) in
                                                match eval.evaluation with
                                                | IntValue(iVal)    -> raise (EvaluationError("Expected value of type bool but got value of type int"))
                                                | FloatValue(fVal)  -> raise (EvaluationError("Expected value of type bool but got value of type float"))
                                                | BoolValue(bVal)   -> evalReturn(eval.newStore, BoolValue(bVal))
                                                | StringValue(sVal) -> raise (EvaluationError("Expected value of type bool but got value of type string"))
and
expression_string_eval x currStore = match x with
    ...
    | Expression_Identifier_To_String(exp) -> let eval = (expression_identifier_eval exp currStore) in
                                                  match eval.evaluation with
                                                  | IntValue(iVal)    -> raise (EvaluationError("Expected value of type string but got value of type int"))
                                                  | FloatValue(fVal)  -> raise (EvaluationError("Expected value of type string but got value of type float"))
                                                  | BoolValue(bVal)   -> raise (EvaluationError("Expected value of type string but got value of type bool"))
                                                  | StringValue(sVal) -> evalReturn(eval.newStore, StringValue(sVal))
and
expression_identifier_eval x currStore = match x with
    | Expression_Identifier_Dereference(iden)           -> let Identifier_Reference(idenName) = iden in evalReturn(currStore, storeLookup currStore idenName)
    ...
    | Expression_Identifier_Assign(iden, exp)           -> let Identifier_Reference(idenName) = iden in
                                                           match (storeLookup currStore idenName) with
...
int_operation_eval x currStore = match x with
    | Operation_Int_Plus_Int(lhs, rhs)            -> let lhsEval = (expression_int_eval lhs currStore) in
                                                     ...
                                                     let IntValue(lhsValue) = lhsEval.evaluation in
                                                     let IntValue(rhsValue) = rhsEval.evaluation in
    ...
    | Operation_Negate_Int(exp)                   -> let expEval = (expression_int_eval exp currStore) in
                                                     let IntValue(expValue) = expEval.evaluation in
    ...
    | Operation_String_Length(exp)                -> let expEval = (expression_string_eval exp currStore) in
                                                     let StringValue(expValue) = expEval.evaluation in
...
float_operation_eval x currStore = match x with
    | Operation_Float_Plus_Float(lhs, rhs)          -> let lhsEval = (expression_float_eval lhs currStore) in
                                                       ...
                                                       let FloatValue(lhsValue) = lhsEval.evaluation in
                                                       let FloatValue(rhsValue) = rhsEval.evaluation in
    ...
    | Operation_Negate_Float(exp)                   -> let expEval = (expression_float_eval exp currStore) in
                                                       let FloatValue(expValue) = expEval.evaluation in
...
bool_operation_eval x currStore = match x with
    | Operation_Bool_And_Bool(lhs, rhs)    -> let lhsEval = (expression_bool_eval lhs currStore) in
                                              ...
                                              let BoolValue(lhsValue) = lhsEval.evaluation in
                                              let BoolValue(rhsValue) = rhsEval.evaluation in
    ...
    | Operation_Not_Bool(exp)              -> let expEval = (expression_bool_eval exp currStore) in
                                              let BoolValue(expValue) = expEval.evaluation in
...
string_operation_eval x currStore = match x with
    | Operation_String_Concat_String(lhs, rhs)  -> let lhsEval = (expression_string_eval lhs currStore) in
                                                   ...
                                                   let StringValue(lhsValue) = lhsEval.evaluation in
                                                   let StringValue(rhsValue) = rhsEval.evaluation in
    ...
    | Operation_Substring_String_Int_Int(strExp, startExp, lenExp) -> let strExpEval = (expression_string_eval strExp currStore) in
                                                                      ...
                                                                      let StringValue(strExpValue) = strExpEval.evaluation in
                                                                      let IntValue(startExpValue) = startExpEval.evaluation in
                                                                      let IntValue(lenExpValue) = lenExpEval.evaluation in
